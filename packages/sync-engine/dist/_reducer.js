// _reducer.ts
import { compareClock } from './types';
// 用 clientTime 作为逻辑时钟；serverTimeMs 仅用于展示或审计
const logicalTime = (e) => e.clientTime;
const clockOf = (e) => ({ t: logicalTime(e), tie: e.opId });
export function foldEvents(events) {
    const state = new Map();
    const version = new Map(); // 每条消息的最后时钟
    for (const ev of events) {
        const clk = clockOf(ev);
        const prev = version.get(ev.messageId);
        if (prev && compareClock(prev, clk) > 0)
            continue; // 老事件丢弃
        if (ev.type === 'create') {
